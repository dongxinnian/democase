...
function toJoin(){
	...
	if(window.mod3D){
		...
		for(var i = 0;i<aA.length;i++){
			clearInterval(aLi[i].dragTimer);
			clearInterval(aLi[i].timer);
			aA[i].style.backgroundImage = 'url(images/'+ window.view +'.jpg)';
...
function picTab2D(num){
	...
	for(var i =0;i<aStrong.length;i++){			//strong记录当前
		...
		aStrong[i].style.backgroundImage = 'url(images/'+ window.view +'.jpg)';
...
module3d.onclick = function(){
	...
	if(window.mod3D){				//2d参数
		...
		window.mod3D = true;
		linkCSS.href = "style/module3d.css";
		for(var i = 0;i<aLi.length;i++){
			clearInterval(aLi[i].timer);
			clearInterval(aLi[i].dragTimer)
		}
